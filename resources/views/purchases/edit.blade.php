...
@section('title', 'Edit Purchase Order #' . $purchase->po_number . ' - PT Nusantara ERP')
...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <!-- Form -->
    <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="poForm">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        id="po_date"
                        name="po_date" 
                        value="{{ old('po_date', $purchase->po_date ? $purchase->po_date->format('Y-m-d') : '') }}" 
                ...
                <div>
                    <label for="status" class="block text-sm font-medium text-gray-700 mb-2">
                        Status PO <span class="text-red-500">*</span>
                    </label>
                    <select 
                        id="status"
                        name="status" 
                    ...
                    >
                        ...
                        <option value="Pending Approval" {{ old('status', $purchase->status) == 'Pending Approval' ? 'selected' : '' }}>Pending Approval</option>
            ...
            <div class="pt-4 border-t border-gray-200 flex items-center justify-end gap-3">
                <a href="{{ route('purchases.show', $purchase) }}" class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition text-sm font-medium">
                    Batal
                </a>
...
<script>
    ...
    function addRow(existingProduct = null) {
        const tbody = document.getElementById('itemsTableBody');
        const row = document.createElement('tr');
        row.className = 'hover:bg-gray-50 transition item-row';
        row.dataset.index = itemIndex;

        const selectedProductId = existingProduct ? existingProduct.product_id : '';
        const initialQty = existingProduct ? existingProduct.quantity : 1;
        const initialPrice = existingProduct ? existingProduct.unit_price : 0;

        let productOptions = '<option value="">-- Pilih Produk --</option>';
        productsData.forEach(p => {
            const isSelected = p.id == selectedProductId ? 'selected' : '';
            productOptions += `<option value="${p.id}" data-price="${p.price}" ${isSelected}>${p.product_code} - ${p.product_name}</option>`;
        });
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-3">
                <input type="number" name="items[${itemIndex}][quantity]" value="${initialQty}" min="1" class="qty-input w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" required>
            </td>
            <td class="px-4 py-3">
                <input type="number" name="items[${itemIndex}][unit_price]" value="${initialPrice}" min="0" step="500" class="price-input w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" required>
            </td>
            <td class="px-4 py-3 font-semibold text-gray-900 subtotal-text">
                Rp ${formatNumber(initialQty * initialPrice)}
            </td>
